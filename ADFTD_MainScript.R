...
# EBV linear epitopes in autoimmunity ------

ppEBV=sapply(pp,function(p){
  x=stringdist(p,EBVepi7, method = "lcs")
  EBVepi7[x<3]
})
sum(lengths(ppEBV)>0)
ppEBV[lengths(ppEBV)>0]

# Embedding ---------------------------------------------------------
## Fig embed -----------------------------------
png(file="graphEmbeddings.png", width=50,height = 90, units = "in", res=75)
  par(mfrow=c(5,2))
...
## Rgl mapping + Blood Group Mapping -----

diadifM0=t(apply(as.data.frame(vertex_attr(Gi$IgM)[4:7]),1, function(l) {
  R=l[1]-l[2]
  G=l[3]-l[2]
  B=l[4]-l[2]
  return(c(R,G,B))
}))
rownames(diadifM0)=names(V(Gi$IgM))

diadifM=apply(diadifM0,2,function(l) {
  X=(l-min(l))/diff(range(l))-0.5
  Y=abs(X)/(0.25+abs(X)^2)*sign(X)
  (Y-min(Y))/diff(range(Y))
})
clM=rgb(diadifM)

diadifG0=t(apply(as.data.frame(vertex_attr(Gi$IgG)[4:7]),1, function(l) {
  R=l[1]-l[2]
  G=l[3]-l[2]
  B=l[4]-l[2]
  return(c(R,G,B))
}))
rownames(diadifG0)=names(V(Gi$IgG))

diadifG=apply(diadifG0,2,function(l) {
  X=(l-min(l))/diff(range(l))-0.5
  Y=abs(X)/(0.25+abs(X)^2)*sign(X)
  (Y-min(Y))/diff(range(Y))
})
...
Gi=lapply(nms, function(iso){
  Gi[[iso]]=set_vertex_attr(Gi[[iso]], name=paste("col4",iso, sep="_"), value=clrGM[[iso]])
})
names(Gi)=c("IgM","IgG")
plotG3D(Gi$IgM, att="col4_IgG",coff=4, smoothcol = F, interact = F)

filename = "graphIgG.html"
saveWidget(rglwidget(), filename)


plotG3D(Gi$IgG, att="col4_IgG",coff=5, smoothcol = F, interact = F, savesc=T)


# Generalization error bound (hyperplain) -----
...
VCterm=sqrt((d*(log(2*n/d)+1))/n)
VCterm

# Projection and separation ----
iscl=unlist(stri_extract_all(names(ppfinal), regex="(?<=:)\\w+"))

silh=sapply(seq_along(ppfinal), function(i){
      cl=ppfinal[[i]]
      nm=names(ppfinal)[i]
      DnPC=t(DnGM[[iscl[i]]][cl,])
      s=cluster::silhouette(diall, dist(DnPC), full=T)
      summary(s)$clus.avg.widths
  })
colnames(silh)=names(ppfinal)
rownames(silh)=c("A","C","F","ND")

silhBS=pbsapply(seq_along(ppfinal), function(i){
  sapply(1:100, function(j){
    cl=ppfinal[[i]]
    nm=names(ppfinal)[i]
    s=cluster::silhouette(diall, dist(t(DnGMBS[[iscl[i]]][[j]][cl,])), full=T)
    summary(s)$clus.avg.widths
  })
})
silhBS=array(silhBS, dim=c(4,100,11), dimnames=list(rownames(silh),1:100,colnames(silh)))

par(mfrow=c(1,11))
par(mai=par("mai")+c(0,-0.2,0,-0.1))
sapply(seq_along(ppfinal),function(i) {
  m=t(silhBS[,,i])
  b=boxplot(m, main=names(ppfinal)[i], las=2, ylim =c(-0.32,0.32)) #ylim=range(silh), 
  pchf=silh[,i]>b$stats[5,]
  points(silh[,i], col=1*(!pchf)+c(2,1,3,4)*pchf, pch=16*pchf+1*(!pchf), cex=2)
})  
...
# Generate adjacency list vi (keep it also in v0 because vi will change)

proct=proc.time()
j=0
v0=lapply(as_adj_list(Gi$IgM), names)
vi=sapply(seq_along(v0),function(i) {
  x=c(names(v0)[i],v0[[i]])
  return(x)
})
names(vi)=names(v0)

print(range(lengths(vi)))

Mxi=-1e3
res=array(0,dim = c(length(v0),100)) # holds clus. crit. for each neighborhood and for each step of the iteration
vres=list()
D=diameter(Gi$IgM, weights=rep(1, ecount(Gi$IgM)))

repeat{
  j=j+1
  print(j)
  
  # calculate clus. crit. x for the diagnoses clusters when mapped to the
  # reactivities of the neighborhood of each vertex (i.e. - sequences of similar
  # reactivity profiles as the one of the vertex picked)

    x=sapply(seq_along(vi), function(i){  
      n=vM[i]
      v=vi[[n]]
      if (length(v)>0) clucri(DnGM$IgM[v,], factor(diall)) else return(0) 
     })
  res[,j]=x
  maxcr=max(x)  # best value of the clustering criterion at the current iteration
  print(maxcr)
  
  # stop if the number of steps (graph distance, path length) exceeds the
  # graph diameter by 4-fold
  
  if (j>4*D) break 
  names(x)=names(vi)
  
  # with each neighborhood find the best clustering adjacent neighborhood 
  # and take the union of that one and the current neighborhood,
  # perform recursive feature elimination to find a subset with (quasi) optimal
  # separation of the diagnoses then save these local feature sets in vi 
  
  plan("multisession", workers=15)
  vi=future_lapply(seq_along(vi), function(i) {
    vx=vi[[i]]
    if (length(vx)>0) {
      n=names(vi)[i]
      nnmx=rank(x[vx])
      v=vx[nnmx>(length(vx)-1)]
      v=union(vx,unique(unlist(vi[v])))
      fs=rownames(rfe(DnGM$IgM[v,], as.factor(diall))[[1]])
      return(fs) } else return(c())
  })
  closeAllConnections()
  print(range(lengths(vi)))
  names(vi)=names(x)

  # keep the local sets produced at each step separately but also iterate. With
  # every step the RFE selects feature sets based on the ones selected in the
  # same neighborhood at the previous step plus those from the best neighborhood.
...
nv=names(vres)
xn=paste(nv,rep(1:28,each=709), sep="_")
names(nv)=xn
rm(xn)

vres0=lapply(vM, function(n){
  c(vres[nv==n])
})


vreslast=lapply(vres0,function(l) l[28])
vreslast=unlist(vreslast, recursive = F)
clcrvrl=sapply(vreslast, function(l){
  clucri(DnGM$IgM[l,], factor(diall))
})
clcrvrl[is.na(clcrvrl)]=0
bestfsM=vreslast[clcrvrl==max(clcrvrl)]

projbstM=cmdscale(dist(t(DnGM$IgM[bestfsM[[1]],])), k=2)
col=rep(c(2,3,4,1),5)
plot(projbstM, col=col, cex=2, pch=16)

## From the clustering -----


ppfM=unique(unlist(ppfinal[1:7]))
ppfG=unique(unlist(ppfinal[8:11]))

clucri(DnGM$IgM[ppfM,], diall)
clucri(DnGM$IgG[ppfG,], diall)

plotMDS(DnGM$IgM[ppfM,], col=col)

fs=rownames(rfe(DnGM$IgM[ppfM,], as.factor(diall))[[1]])
clucri(DnGM$IgM[fs,], diall)
plotMDS(DnGM$IgM[fs,], col=col)

fs=rownames(rfe(DnGM$IgG[ppfG,], as.factor(diall))[[1]])
clucri(DnGM$IgG[fs,], diall)

mM=DnGM$IgM[ppfM,]
mG=DnGM$IgG[ppfG,]
rownames(mM)=paste(rownames(mM),"M", sep="_")
rownames(mG)=paste(rownames(mG),"G", sep="_")
mm=rbind((mM-min(mM))/(diff(range(mM))),(mG-min(mG))/(diff(range(mG))))
fs=rownames(rfe(mm, as.factor(diall))[[1]])
clucri(mm[fs,], diall)

plot3d(cmdscale(dist(t(mm[fs,])),3), size=10, col=col, xlab="D1", ylab="D2",zlab="D3")
snapshot3d(filename="ppfinrfeM_3d_1.png")

pcamM=PCA(t(mM), ncp=19)
pcamG=PCA(t(mG), ncp=19)
par(mfrow=c(1,2))
col=as.factor(rep(c(1,2,3,4),5))
#fviz_screeplot(pcamM)
fviz_pca_ind(pcamM, axes = c(1,2),  palette=c("red","green","blue","black"), col.ind = col)
#fviz_screeplot(pcamG)
fviz_pca_ind(pcamG, axes = c(1,2),  palette=c("red","green","blue","black"), col.ind = col)
